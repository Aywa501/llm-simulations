...
OUTPUT_PASS2 = DATA_DIR / "study_enriched_aggregate_pass2.jsonl"
...
def find_pdfs_for_study(seq_id: int) -> list[Path]:
    """
    Find all primary PDFs for a study across Papers/ subdirectories.
    ...
    if not PAPERS_ROOT.exists():
        return found
    for sub in sorted(PAPERS_ROOT.iterdir()):
        if not sub.is_dir():
            continue
        for f in sorted(sub.iterdir()):
            if f.suffix != ".pdf":
                continue
            stem = f.stem  # e.g. "19", "30_1", "119_2"
            if stem == str(seq_id):
                found.append(f)
...
INSTRUMENT_SYSTEM = (
    ...
    "TREATMENT_VARIATIONS: the arm-specific text that differs across conditions. "
