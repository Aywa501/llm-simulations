...
CACHE_PATH   = DATA_DIR / "Caches" / ".extract_gt_v2_cache.json"
...
def load_studies() -> dict[int, dict]:
    """
    Reads simulatable_studies.jsonl (JSONL format, one study per line).
    Normalises each record so treatment_variations and outcome_questions
    are always top-level, regardless of whether they're wrapped under
    an 'instrument' key (new format) or at root (old format).
    Also surfaces arm preamble/text and outcome response_format from
    wherever they live in the record.
    """
    ...
    with open(STUDIES_PATH) as f:
        for line in f:
            ...
            # New format wraps everything under 'instrument'
            inst = rec.get("instrument") or {}
            variations = inst.get("treatment_variations") or rec.get("treatment_variations", [])
            out_qs     = inst.get("outcome_questions")     or rec.get("outcome_questions", [])
            preamble   = inst.get("preamble")              or rec.get("preamble", "")

            studies[seq_id] = {
                "seq_id":               seq_id,
                "title":                rec.get("title", f"seq={seq_id}"),
                "preamble":             preamble,
                "treatment_variations": variations,
                "outcome_questions":    out_qs,
            }
    return studies
